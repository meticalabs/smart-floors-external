...
def arg_parser(args):
    ...
    parser.add_argument("--region", required=True, type=str,
                        help="Region to load config (str). Ex: us-east-1, eu-west-1")
    parser.add_argument('--s3DataBucket', help='S3 bucket name for all events and bid floor training data')
    parser.add_argument("--icebergCatalog", required=True, type=str,
                        help="Iceberg catalog name (str). Ex: iceberg_catalog.")
...
class Events:
    ...
    def __post_init__(self):
        self.iceberg_io = IcebergIO(spark=self.spark, region_name=self.region_name,
                                    iceberg_catalog=self.iceberg_catalog)
        self.maintenance = Maintenance(spark=self.spark, iceberg_catalog=self.iceberg_catalog)
        self.training_data_table_config = TableConfig(format="iceberg", database_name="applovin",
                                                      table_name="bid_floor_training_data")
        ...
        self.ad_revenue_columns = [Schema.REQUEST_ID, Schema.USER_ID, Schema.CPM_FLOOR_AD_UNIT_ID,
                                   Schema.TOTAL_AMOUNT, Schema.DATE]

        self.bid_sequence_columns = [Schema.REQUEST_ID, Schema.USER_ID, Schema.IS_FILLED,
                                     Schema.CPM_FLOOR_AD_UNIT_ID, Schema.CPM_FLOOR_VALUE, Schema.DATE]
    ...
    def fetch_assignment_events(self):
        return self.read_events_parquet(event_name="metica_user_assignment").filter(
            col(Schema.DATE).__le__(self.date_iso))

    def fetch_revenue_events(self):
        return self.read_events_parquet(event_name="estimated_ad_revenue",
                                        columns=self.ad_revenue_columns).filter(
            col(Schema.DATE).__le__(self.date_iso)).drop(Schema.DATE)

    def fetch_bid_sequence_events(self):
        return self.read_events_parquet(event_name="applovin_bid_floor",
                                        columns=self.bid_sequence_columns).filter(
            col(Schema.IS_FILLED).__and__(col(Schema.DATE).__le__(self.date_iso))).drop(Schema.DATE)

    def join_all(self, assignment_data: DataFrame, bid_sequence_data: DataFrame, ad_revenue_data: DataFrame):
        merged_df = assignment_data.join(ad_revenue_data.select(self.ad_revenue_columns).drop(Schema.DATE),
                                         on=[Schema.REQUEST_ID, Schema.USER_ID], how='inner')
        return (merged_df.join(bid_sequence_data.select(self.bid_sequence_columns).drop(Schema.DATE),
                               on=[Schema.REQUEST_ID, Schema.USER_ID, Schema.CPM_FLOOR_AD_UNIT_ID],
                               how='inner').withColumns({Schema.EVENT_TIME: col(Schema.EVENT_TIME).cast("timestamp"),
...
def extract_events(spark: SparkSession, logger: logging.Logger, args: [str]):
    ...
    events = Events(customer_id=parsed_args_obj.customerId, app_id=parsed_args_obj.appId,
                    s3_data_bucket=parsed_args_obj.s3DataBucket, date=datetime.date.fromisoformat(parsed_args_obj.date),
                    spark=spark,
                    iceberg_catalog=parsed_args_obj.icebergCatalog, region_name=parsed_args_obj.region, logger=logger)
